...
interface FileOperationDialogProps {
  ...
  folders: Folder[]
  onAction: (item: File | Folder, destinationFolderId: string | null, ...args: any[]) => Promise<any>
...
export function FileOperationDialog({
  ...
  const handleAction = async () => {
    ...
    try {
      ...
      setError(`Failed to ${actionButtonText.toLowerCase()} item. Please try again.`)
    } finally {
    ...
    <Dialog open={open} onOpenChange={(open) => !isProcessing && onOpenChange(open)}>
      <DialogContent className="sm:max-w-[500px]">
        ...
        <div className="space-y-4 py-2">
          ...
          <div className="space-y-2">
            ...
            <div className="border rounded-md h-60 overflow-y-auto p-2">
              <div
                ...
                tabIndex={0}
                role="button"
                aria-label="Select My Drive (root folder)"
              >
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Enter a path like "/Work Documents/Projects" to {itemType === "file" ? "place" : "move"} to a specific
              location.
            </p>
          </div>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleAction} disabled={isProcessing || (!selectedFolderId && !customPath)}>
            {isProcessing ? (
...
function FolderItem({
  ...
  const defaultIsValidDestination = (folder: Folder, currentItemId: string): boolean => {
    ...
    <div className="ml-2">
      <div
        className={`flex items-center p-2 rounded-md ${
          !isValid ? "opacity-50 cursor-not-allowed" : selectedFolderId === folder.id ? "bg-muted" : "hover:bg-muted/50"
        }`}
        ...
        role="button"
        aria-label={`Select folder: ${folder.name}${isParentOfCurrentItem ? " (current)" : ""}`}
      >
        {folder.hasChildren && (
          <ChevronRight
