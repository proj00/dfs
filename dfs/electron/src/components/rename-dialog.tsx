...
export function RenameDialog({
  ...
  const extension =
    item && itemType === "file" ? getFileExtension(item.name) : "";
  const nameWithoutExtension =
    itemType === "file" ? getFileNameWithoutExtension(newName) : newName;

  return (
    <Dialog
      open={open}
      onOpenChange={(open) => !isRenaming && onOpenChange(open)}
    >
      <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            <div className="col-span-3 flex items-center">
              <Input
                ...
                className={
                  itemType === "file" && extension ? "rounded-r-none" : ""
                }
                autoFocus
                onKeyDown={(e) => {
                  if (e.key === "Enter") {
        ...
        <DialogFooter>
          ...
          <Button
            ...
            disabled={isRenaming || !newName.trim() || newName === item?.name}
          >
            {isRenaming ? (
