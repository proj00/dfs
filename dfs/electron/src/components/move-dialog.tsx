...
export function MoveDialog({ open, onOpenChange, item, folders, onMove }: MoveDialogProps) {
    ...
    const handleMove = async () => {
        ...
        try {
            ...
            // For now, we'll just use the selected folder ID
            await onMove(item, destinationId)
            onOpenChange(false)
        } catch (err) {
    ...
    const getCurrentLocation = () => {
        ...
        // In a real app, you might want to show all parent folders
    ...
    return (
        <Dialog open={open} onOpenChange={(open) => !isMoving && onOpenChange(open)}>
            <DialogContent className="sm:max-w-[500px]">
                <DialogHeader>
                    <DialogTitle>Move {item?.name}</DialogTitle>
                    <DialogDescription>
                        Select a destination folder or enter a custom path to move this{" "}
                        {item?.hasOwnProperty("size") ? "file" : "folder"}.
                    </DialogDescription>
                </DialogHeader>
                ...
                <div className="space-y-4 py-2">
                    ...
                    <div className="space-y-2">
                        ...
                        <p className="text-xs text-muted-foreground">
                            Enter a path like "/Work Documents/Projects" to move to a specific location.
                        </p>
                    </div>
                </div>
                ...
                <DialogFooter>
                    ...
                    <Button
                        ...
                        disabled={
                            isMoving ||
                            (!selectedFolderId && !customPath) ||
                            (item?.hasOwnProperty("hasChildren") && item.parentId.includes(selectedFolderId || ""))
                        }
                    >
                        {isMoving ? (
...
function FolderItem({ folder, folders, selectedFolderId, setSelectedFolderId, level, currentItemId }: FolderItemProps) {
    ...
    const isDescendantOfCurrentItem = (folderId: string, itemId: string): boolean => {
        ...
        return folder.parentId.some((parentId) => isDescendantOfCurrentItem(parentId, itemId))
    }

    const isInvalidDestination =
        isParentOfCurrentItem ||
        (currentItemId && folder.hasOwnProperty("hasChildren") && isDescendantOfCurrentItem(folder.id, currentItemId))

    return (
        <div className="ml-2">
            <div
                className={`flex items-center p-2 rounded-md cursor-pointer ${isInvalidDestination
                        ? "opacity-50 cursor-not-allowed"
                        : selectedFolderId === folder.id
                            ? "bg-muted"
                            : "hover:bg-muted/50"
                    }`}
                onClick={() => !isInvalidDestination && setSelectedFolderId(folder.id)}
            >
                {folder.hasChildren && (
                    <ChevronRight
                ...
                <FolderOpen className="h-5 w-5 mr-2 text-blue-500" />
                <span className={isInvalidDestination ? "text-muted-foreground" : ""}>
                    {folder.name}
