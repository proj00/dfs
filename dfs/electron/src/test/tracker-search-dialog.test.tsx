...
jest.mock("@/IpcService/GetNodeService", () => ({
  GetNodeService: jest.fn().mockResolvedValue({
    SearchForObjects: jest.fn().mockResolvedValue({
      responses: [
        {
          name: "Test Container",
...
describe("TrackerSearchDialog", () => {
  ...
  it("handles copying container GUID", async () => {
    ...
    expect(navigator.clipboard.writeText).toHaveBeenCalledWith("container-guid-123")
  })

  it("calls onDownloadContainer when download button is clicked", async () => {
    const onDownloadContainer = jest.fn()

    render(<TrackerSearchDialog open={true} onOpenChange={() => {}} onDownloadContainer={onDownloadContainer} />)
    ...
    expect(onDownloadContainer).toHaveBeenCalledWith("container-guid-123", "tracker.example.com")
  })

  it("handles search errors", async () => {
    // Override the mock to simulate an error
    ;(GetNodeService as jest.Mock).mockResolvedValue({
      SearchForObjects: jest.fn().mockRejectedValue(new Error("Search failed")),
    })
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Failed to search tracker. Please check the tracker URI and try again."),
      ).toBeInTheDocument()
    })
  })
})
