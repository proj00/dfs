...
await act (async () => {
  ...
  fireEvent.click(whitelistTabTrigger);
    ...
    await waitFor(() => {
      ...
      expect(allTabTrigger).toHaveAttribute('data-state', 'inactive'); // Ensure other tab is inactive

      // Whitelisted item should be present
      expect(screen.getByText("2001:db8::/64")).toBeInTheDocument();
      // Blacklisted items should NOT be present
      expect(screen.queryByText("192.168.1.1")).not.toBeInTheDocument(); // This was the failing line
    ...
    await waitFor(() => {
      expect(blacklistTabTrigger).toHaveAttribute('data-state', 'active');
      expect(whitelistTabTrigger).toHaveAttribute('data-state', 'inactive');
