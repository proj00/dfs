...
jest.mock("@/IpcService/GetNodeService", () => ({
  GetNodeService: jest.fn().mockResolvedValue({
    ...
    DownloadContainer: jest.fn().mockResolvedValue(["download-id-1", "download-id-2"]),
...
jest.mock("../components/download-manager", () => ({
  DownloadManager: jest.fn(() => <div data-testid="download-manager">Download Manager</div>),
}))

// Mock the BlockedPeersDialog component
jest.mock("../components/blocked-peers-dialog", () => ({
  BlockedPeersDialog: jest.fn(() => <div data-testid="blocked-peers-dialog">Blocked Peers Dialog</div>),
}))
...
describe("Sidebar", () => {
  ...
  it("opens the publish dialog when Publish to Tracker is clicked", () => {
    ...
    expect(
      screen.getByText("Select a container and enter a tracker URI to publish your container."),
    ).toBeInTheDocument()
  })
  ...
  it("opens the download dialog when Download Container is clicked", () => {
    ...
    // Verify dialog is opened
    expect(screen.getByText("Enter a container GUID and tracker URI to download a container.")).toBeInTheDocument()
  })

  it("publishes a container when form is submitted", async () => {
    const onContainerPublished = jest.fn()

    render(<Sidebar {...defaultProps} onContainerPublished={onContainerPublished} />)
  ...
  it("downloads a container when form is submitted", async () => {
    ...
    render(<Sidebar {...defaultProps} onContainerDownloaded={onContainerDownloaded} />)
