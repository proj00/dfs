...
export namespace fs {
  ...
  export class ObjectList extends pb_1.Message {
    #one_of_decls: number[][] = [];
    constructor(
      data?:
        | any[]
    ...
    static deserialize(bytes: Uint8Array | pb_1.BinaryReader): ObjectList {
      const reader =
          bytes instanceof pb_1.BinaryReader
            ? bytes
            : new pb_1.BinaryReader(bytes),
        message = new ObjectList();
      while (reader.nextField()) {
        if (reader.isEndGroup()) break;
        switch (reader.getFieldNumber()) {
          case 1:
    ...
    static deserializeBinary(bytes: Uint8Array): ObjectList {
      return ObjectList.deserialize(bytes);
    }
  }
}
