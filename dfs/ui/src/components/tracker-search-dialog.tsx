...
export function TrackerSearchDialog({ open, onOpenChange, onDownloadContainer }: TrackerSearchDialogProps) {
    ...
    const handleSearch = async () => {
        ...
        try {
            ...
            const results = await nodeService.SearchForObjects(searchQuery, trackerUri);
            ...
            setError("Failed to search tracker. Please check the tracker URI and try again.")
        } finally {
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[600px] max-h-[80vh] overflow-hidden flex flex-col">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Search for objects on a tracker by entering a tracker URI and search query.
                    </DialogDescription>
                </DialogHeader>
                ...
                <div className="grid gap-4 py-4">
                    ...
                    <div className="grid grid-cols-4 items-center gap-4">
                        ...
                        <div className="col-span-3 flex gap-2">
                            ...
                            />
                            <Button onClick={handleSearch} disabled={!trackerUri || !searchQuery || isSearching}>
                                {isSearching ? (
                ...
                <div className="flex-1 overflow-auto">
                    {searchResults.length === 0 && !isSearching && !error ? (
                        <div className="flex flex-col items-center justify-center h-64 text-muted-foreground">
                            ...
                            <p className="text-sm mt-2">Enter a tracker URI and search query to find objects</p>
                        </div>
                    ) : (
                            ...
                            {searchResults.map((result, index) => (
                                <div key={index} className="border rounded-md p-4 space-y-2">
                                    <div className="flex justify-between items-start">
                                        <div>
                                            <h3 className="font-medium">{result.name || "Unnamed Container"}</h3>
                                            <p className="text-sm text-muted-foreground truncate">{result.containerGuid}</p>
                                        </div>
                                        <TooltipProvider>
                                            <ToolTip>
                                                <TooltipTrigger asChild>
                                                    <Button variant="ghost" size="icon" onClick={() => handleCopyGuid(result.containerGuid)}>
                                    ...
                                    <div className="flex justify-between items-center pt-2">
                                        <div className="flex items-center text-sm text-muted-foreground">
                                            ...
                                            <span>Size: {result.size ? formatFileSize(Number(result.size)) : "Unknown"}</span>
                                        </div>
                                        <Button
                                            size="sm"
